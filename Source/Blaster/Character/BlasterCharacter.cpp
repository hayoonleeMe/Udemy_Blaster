...
ABlasterCharacter::ABlasterCharacter()
{
	...
	// 서버에서만 사용하므로 Replicated될 필요가 없다.
...
{
	if (GetMesh() == nullptr || OriginalMaterial == nullptr || BlueDissolveMatInst == nullptr || BlueMaterial == nullptr || RedDissolveMatInst == nullptr || RedMaterial == nullptr) return;
...
void ABlasterCharacter::RotateInPlace(float DeltaTime)
{
	...
	// 무기를 착용 중이고 HoldingTheFlag 중이 아닐 때의 움직임 설정
...
void ABlasterCharacter::PlayReloadMontage()
{
	...
	bool bHideSniperScope = IsLocallyControlled() && Combat->bAiming && Combat->EquippedWeapon->GetWeaponType() == EWeaponType::EWT_SniperRifle; 
	if (bHideSniperScope)
	{
		AimButtonReleased();
	}
...
{
	// Locally Controlled 되는 캐릭터만 Input을 받을 수 있다.
                                      ...
                                      AController* InstigatorController, AActor* DamageCauser)
{
	...
	UpdateHUDHealth();
	UpdateHUDShield();
	if (Damage != 0.f) PlayHitReactMontage();
...
void ABlasterCharacter::HideCameraIfCharacterClose()
{
	...
	if ((FollowCamera->GetComponentLocation() - GetActorLocation()).Size() < CameraThreshold)
	{
		...
		{
			Combat->EquippedWeapon->GetWeaponMesh()->bOwnerNoSee = true;		// Owner만 볼 수 없게된다.
		}
		if (Combat && Combat->SecondaryWeapon && Combat->SecondaryWeapon->GetWeaponMesh())
        {
        	Combat->SecondaryWeapon->GetWeaponMesh()->bOwnerNoSee = true;		// Owner만 볼 수 없게된다.
        }
	}
	else
	{
...
{
	// Dissolve Curve 를 사용하기 위한 셋업
	// Curve 를 Timeline 에 추가하고, Bind 된 콜백 함수에 Curve 의 결과값을 전달하며 매 프레임 호출한다.
...
void ABlasterCharacter::ReloadFinished()
{
	if (Combat)
	{
