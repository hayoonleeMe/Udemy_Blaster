	...
	ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		...
		{
			if (ConfirmHitResult.Component.IsValid())	// TWeakObjectPtr는 고유의 IsValid() 함수를 이용해 Valid를 체크해야한다.
			{
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	{
		if (PathResult.HitResult.Component.IsValid())	// TWeakObjectPtr는 고유의 IsValid() 함수를 이용해 Valid를 체크해야한다.
		{
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (Confirm.bHitConfirmed && Character && Character->GetEquippedWeapon() && HitCharacter)
	{
	...
	const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	for (auto& HitCharacter : HitCharacters)
	{
		...
		{
			float HeadShotDamage = Confirm.HeadShots[HitCharacter] * Character->GetEquippedWeapon()->GetDamage();	// 아직 헤드샷 데미지 구현 X
